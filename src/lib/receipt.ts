...
const ReceiptDeliverySummarySchema = z.object({
  state: z.literal("sent"),
  resourceType: z.literal("bank_transaction_explanation"),
  label: z.string().min(1).max(160),
  sentAt: z.unknown(),
});
...
export const ReceiptSchema = z.discriminatedUnion("status", [
  ...
  ProcessedReceiptSchema.extend({
    ...
    delivery: ReceiptDeliverySummarySchema.optional(),
  }),
]);
...
export type ReceiptQueue = "needs_review" | "extracting" | "ready_to_verify" | "ready_to_match" | "proposed" | "sent" | "problems" | "original";
...
export function receiptQueue(receipt: Receipt): ReceiptQueue {
  ...
  if (receipt.status === "verified") {
    if (receipt.delivery) return "sent";
    return receipt.matching ? "proposed" : "ready_to_match";
  }
